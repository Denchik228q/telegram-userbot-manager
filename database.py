...
class Database:
    ...
    async def update_user_phone(self, user_id: int, phone: str):
        """Обновить телефон"""
        try:
            await self.db.execute('UPDATE users SET phone = ? WHERE user_id = ?', (phone, user_id))
            await self.db.commit()
        except Exception as e:
    ...
    async def check_subscription(self, user_id: int):
        ...
        try:
            async with self.db.execute(
                'SELECT subscription_type, subscription_end FROM users WHERE user_id = ?',
                (user_id,)
            ) as cursor:
    ...
    async def activate_subscription(self, user_id: int, sub_type: str):
        ...
        try:
            ...
            end_date = (datetime.now() + timedelta(days=sub_data['duration_days'])).isoformat()
    ...
    async def increment_messages_sent(self, user_id: int):
        ...
        try:
            await self.db.execute(
                'UPDATE users SET messages_sent_today = messages_sent_today + 1 WHERE user_id = ?',
                (user_id,)
            )
            await self.db.commit()
        except Exception as e:
