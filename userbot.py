...
class UserbotManager:
    ...
    async def can_send_messages(self, client, target: str):
        ...
        try:
            # Пропускаем приватные инвайт-ссылки
            if 't.me/+' in target or 't.me/joinchat/' in target:
                logger.warning(f"⚠️ Skipping invite link check: {target}")
                return False
            ...
            if isinstance(entity, User):
                logger.info(f"✅ {target_clean} is User - can write")
                return True
            
            if isinstance(entity, Chat):
                logger.info(f"✅ {target_clean} is Chat - can write")
                return True
            ...
            if isinstance(entity, Channel):
                try:
                    ...
                    if hasattr(permissions, 'is_banned') and permissions.is_banned:
                        logger.warning(f"❌ {target_clean} - user is BANNED")
                        return False
                    
                    if hasattr(permissions, 'send_messages'):
                        can_send = permissions.send_messages
                        logger.info(f"{'✅' if can_send else '❌'} {target_clean} - send_messages={can_send}")
                        return can_send
                    
                    if entity.broadcast:
                        logger.warning(f"❌ {target_clean} - is broadcast channel (admins only)")
                        return False
                    
                    if entity.default_banned_rights:
                        can_send = not entity.default_banned_rights.send_messages
                        logger.info(f"{'✅' if can_send else '❌'} {target_clean} - default rights: {can_send}")
                        return can_send
                    
                    logger.info(f"✅ {target_clean} - no restrictions")
                    return True
                    
                except Exception as perm_err:
                    logger.error(f"⚠️ Can't check permissions for {target_clean}: {perm_err}")
                    return True
            
            logger.warning(f"⚠️ {target_clean} - unknown type: {entity.__class__.__name__}")
            return True
    ...
    async def send_photo(self, session_id: str, phone: str, target: str, photo_path: str, caption: str = ""):
        ...
        try:
            ...
            logger.info(f"🔄 Attempting to send photo to: {target_clean}")
